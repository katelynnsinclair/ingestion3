...
/**
  ...
  */
class DCFileExtractor extends XmlExtractor

/**
  * Entry for harvesting XML serialized from an OAI endpoint
  */
class DCFileHarvester(spark: SparkSession,
                      shortName: String,
                      conf: i3Conf,
                      logger: Logger)
      ...
      val recordCount = (for (result <- iter(inputStream)) yield {
        handleFile(result, unixEpoch) match {
          case Failure(exception) =>
            logger.error(s"Caught exception on $inFile. ${exception.getMessage}")
            0
          case Success(count) =>
            count
        }
