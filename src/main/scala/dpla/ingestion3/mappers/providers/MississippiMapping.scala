...
class MississippiMapping extends JsonMapping with JsonExtractor with IngestMessageTemplates {
  ...
  override def isShownAt(data: Document[JValue]): ZeroToMany[EdmWebResource] =
    extractStrings(unwrap(data) \ "delivery" \ "availabilityLinksUrl").map(stringOnlyWebResource)

  override def preview(data: Document[JValue]): ZeroToMany[EdmWebResource] = {
    (unwrap(data) \ "delivery" \ "link")
      .filter(node => extractString(node \ "displayLabel").getOrElse("").equalsIgnoreCase("thumbnail"))
  ...
  override def creator(data: Document[JValue]): ZeroToMany[EdmAgent] =
    extractStrings(unwrap(data) \ "pnx" \ "display" \ "creator").map(nameOnlyAgent)

  override def contributor(data: Document[JValue]): ZeroToMany[EdmAgent] =
    extractStrings(unwrap(data) \ "pnx" \ "display" \ "contributor").map(nameOnlyAgent)
  ...
  override def publisher(data: Document[JValue]): ZeroToMany[EdmAgent] =
    extractStrings(unwrap(data) \ "pnx" \ "display" \ "publisher").map(nameOnlyAgent)
  ...
  override def subject(data: Document[JValue]): ZeroToMany[SkosConcept] =
    extractStrings(unwrap(data)  \ "pnx" \ "display" \ "subject").map(nameOnlyConcept)

  override def date(data: Document[JValue]): ZeroToMany[EdmTimeSpan] =
     extractStrings(unwrap(data) \ "pnx" \ "display" \ "creationdate").map(stringOnlyTimeSpan)
